...
EvaluableNodeReference Parser::ParseFromKeyString(std::string &code_string, EvaluableNodeManager *enm)
{
	...
	else
	{
		...
		else if(tree->IsOrderedArray())
		{
						...
						== OpcodeDetails::ChildNodeStructureType::ONE_POSITION_THEN_UNORDERED_OR_ONE_UNORDERED))
			{
				...
				std::stable_sort(begin(unordered_code) + std::min(starting_index, unordered_code.size()),
