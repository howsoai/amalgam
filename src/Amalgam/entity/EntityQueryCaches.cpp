		...
		case ENT_QUERY_AMONG:
		{
			if(is_first)
			{
				for(size_t i = 0; i < cond->valueToCompare.size(); i++)
					sbfds.UnionAllEntitiesWithValue(cond->singleLabel,
						EvaluableNodeImmediateValueWithType(cond->valueToCompare[i], cond->valueTypes[i]), matching_entities);
			}
			else
			{
				...
				for(size_t i = 0; i < cond->valueToCompare.size(); i++)
					sbfds.UnionAllEntitiesWithValue(cond->singleLabel,
						EvaluableNodeImmediateValueWithType(cond->valueToCompare[i], cond->valueTypes[i]), temp);
...
void EntityQueryCaches::GetMatchingEntitiesViaSamplingWithReplacement(EntityQueryCondition *cond, BitArrayIntegerSet &matching_entities, std::vector<size_t> &entity_indices_sampled, bool is_first, bool update_matching_entities)
{
	...
	else //sampling a bunch, better to precompute and use faster method
	{
		...
		WeightedDiscreteRandomStreamTransform<size_t> ewt(entity_indices, probabilities, false);
			...
			if(exist_labels.size() > 0)
			{
				...
				for(const auto &entity_index : matching_ents)
				{
					...
					for(auto &label_sid : exist_labels)
						entity_values->SetMappedChildNodeWithReferenceHandoff(label_sid, contained_entities[entity_index]->GetValueAtLabel(label_sid, enm).first);
