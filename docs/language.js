...
*/
...

	{
		...
		"new value" : "new",
		"new target scope": true,
		"description" : "For each element in the collection after the first one, it pushes a pair of new target scope onto the stack, so that current_value accesses a list of elements from the list, and current_index accesses the list or assoc index if it is not already reduced, with target representing the original list or assoc, and evaluates function. If the collection is empty, null is returned. if the collection is of size one, the single element is returned.",
		"example" : "(print (reduce (lambda (* (current_value 1) (current_value))) (list 1 2 3 4)))"
	},
