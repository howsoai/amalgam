...
namespace EntityQueryBuilder
{
		...
		std::vector<EntityQueryCondition> &conditions, RandomStream &rs)
	{
		...
		if(cur_condition->hasRandomStream)
			cur_condition->randomStream.SetState(EvaluableNode::ToKeyString(ocn[RANDOM_SEED]));
		else
		...
		std::vector<EntityQueryCondition> &conditions, RandomStream &rs)
	{
		...
		switch(type)
		{
			case ENT_QUERY_SELECT:
			{
				...
				if(cur_condition->hasRandomStream)
					cur_condition->randomStream.SetState(EvaluableNode::ToKeyString(ocn[2]));
				else
					cur_condition->randomStream = rs.CreateOtherStreamViaRand();
			...
			case ENT_QUERY_SAMPLE:
			{
				...
				if(cur_condition->hasRandomStream)
					cur_condition->randomStream.SetState(EvaluableNode::ToKeyString(ocn[1]));
				else
					cur_condition->randomStream = rs.CreateOtherStreamViaRand();
			    break;
			}
