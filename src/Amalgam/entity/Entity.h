...
public:
	...
	std::string CreateRandomStreamFromStringAndRand(std::string_view seed_string);
	...
	inline static bool IsNamedEntity(std::string_view &id)
	{
	...
	{
		auto id_name = string_intern_pool.GetStringViewFromID(id);
	...
	static inline bool IsLabelValidAndPublic(StringInternPool::StringID label_sid)
	{
		...
		auto label_name = string_intern_pool.GetStringViewFromID(label_sid);
	...
	static inline bool IsLabelValidAndPublic(std::string_view label_name)
	{
	...
	//same as the same-named function with StringInternPool::StringID but with actual string
	static inline bool IsLabelPrivate(std::string_view label_name)
	{
		if(label_name.size() == 0)
			return false;
		if(label_name[0] == '!')
	...
	//same as the same-named function with StringInternPool::StringID but with actual string
	static inline bool IsLabelAccessibleToContainedEntities(std::string_view label_name)
	{
		if(label_name.size() == 0)
			return false;
		if(label_name[0] == '^')
