...
#### Parameters
`query_entity_clusters number num_neighbors list feature_labels number min_cluster_weight [number p_value] [list|assoc|assoc of assoc weights] [list|assoc distance_types] [list|assoc attributes] [list|assoc deviations] [string weights_selection_feature] [string|number distance_transform] [string entity_weight_label_name] [number random_seed] [string radius_label] [string numerical_precision] [* output_sorted_list]`
#### Description
When used as a query argument, computes a cluster id for each of the entities using HDBSCAN.  num_neighbors is the number of nearest neighbors used to estimate each entity's local density (its core distance); it should be smaller than the expected cluster size.  min_cluster_weight is the smallest accumulated entity weight that will be considered a cluster, and can be 0.  The ids are nonnegative integers, with the id of zero denoting entities that are independent and isolated from all clusters (noise).  feature_labels specifies the names of the features to consider during computation.  See Distance and Surprisal Calculations for details on the other parameters and how distance is computed.  If output_sorted_list is not specified or is false, then it will return an assoc of entity string id as the key with the cluster id as the value; if output_sorted_list is true, then it will return a list of lists, where the first list is the entity ids and the second list contains the corresponding cluster ids, both in order of ascending cluster id. If output_sorted_list is a string, then it will additionally return a list where the values correspond to the values of the labels for each respective entity.  If output_sorted_list is a list of strings, then it will additionally return a list of values for each of the label values for each respective entity.
#### Details
 - Permissions required:  none
 - Allows concurrency: true
 - Requires entity: false
