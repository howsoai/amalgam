...
public:
	...
	enum class Permission : RequestType
	{
		NONE = 0,
	...
	explicit inline ExecutionPermissions(Permission initial_permissions)
		: allPermissions(static_cast<RequestType>(initial_permissions))
	{}

	inline bool HasPermission(Permission permission) const
	{
		return (allPermissions & static_cast<RequestType>(permission)) != 0;
	}

	inline void SetPermission(Permission permission, bool enable = true)
	{
		if(enable)
			allPermissions |= static_cast<RequestType>(permission);
		else
			allPermissions &= ~static_cast<RequestType>(permission);
	}
	...
	RequestType allPermissions = static_cast<RequestType>(Permission::NONE);
};
