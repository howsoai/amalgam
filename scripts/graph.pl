...
{
   foreach my $file (`find $dir -name '*.cpp' -o -name '*.h' -o -name '*.hpp' -o -name '*.mm'`) {
      my $short = $file;
      chop $short;
      $short =~ s|\.cpp$||;
      $short =~ s|\.h$||;
...
my $grepcmd = "grep -E '^[ \t]*#[ \t]*include[^\"]*\"[^\"]*\\.h(pp)?\"'"; # find include directives with quotes
my $sedcmd = "sed -E 's|^[^\"]*\"([^\"]*)\\.h(pp)?\".*\$|\\1|'"; # extract quoted path
