...
If an operator is preceded by an `@` symbol, then it will be evaluated on load rather than during execution, but only on a limited set of opcodes.  The `@` symbol is particularly useful for storing self-referential or graph data structures or for loading additional code as modules.  The opcodes `@` is permitted on consists of `target`, `get`, `append`, and `load`, and the opcodes' parameters must be resolvable without additional code execution.  The opcodes `target` and `get` can reference locally or from the top of the stack.  For example, `@(target .true "inlined_referenced_method")` will point directly to the top level inlined_reference_method for an inline-like method.  Opcodes that serialize data structures such as `unparse` and `print` will safely handle self-referential data structures and will serialize using `@` symbols appropriately.  The opcodes `load` and `append`, when preceded by `@`, are designed to make it easy to include modules.  The common idiom for including modules within an entity is
```amalgam
@(append
@(load "module_1.amlg")
@(load "module_2.amlg")
...
static std::string_view _help_idioms(R"IDIOMS(# Amalgam Idioms
