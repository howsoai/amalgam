	...
	std::string default_random_seed, Interpreter *calling_interpreter, EntityExternalInterface::LoadEntityStatus &status)
{
	...
	if(file_type == FILE_EXTENSION_COMPRESSED_AMALGAM_CODE)
	{
		...
		args->SetMappedChildNode(GetStringIdFromNodeTypeFromString(ENBISI_create_new_entity), new_entity->evaluableNodeManager.AllocNode(ENT_FALSE));
	...
	if(metadata_status.loaded)
	{
		...
		{
			EvaluableNode **seed = metadata->GetMappedChildNode(GetStringIdFromNodeTypeFromString(ENBISI_rand_seed));
			if(seed != nullptr)
			{
			...
			EvaluableNode **version = metadata->GetMappedChildNode(GetStringIdFromNodeTypeFromString(ENBISI_version));
			if(version != nullptr)
			{
