	...
	std::string &resource_base_path, std::string &file_type, EvaluableNodeManager *enm, bool escape_filename, bool sort_keys)
{
	...
	else if(file_type == FILE_EXTENSION_COMPRESSED_STRING_LIST)
	{
		...
		if(code != nullptr)
		{
			for(auto &cn : code->GetOrderedChildNodes())
				string_map[EvaluableNode::ToOpcodeString(cn)] = cur_index++;
		}
	...
	{
		std::string s = EvaluableNode::ToOpcodeString(code);
	...
	bool persistent, bool load_contained_entities, bool escape_filename, bool escape_contained_filenames, std::string default_random_seed)
{
	...
	if(metadata != nullptr)
	{
		if(EvaluableNode::IsAssociativeArray(metadata))
		{
			...
			if(seed != nullptr)
				default_random_seed = EvaluableNode::ToOpcodeString(*seed);
		}
	}
