...
std::pair<std::string, std::string> StringifyNode(EvaluableNode *en, EvaluableNodeManager *enm, size_t max_num_chars = 100)
{
	...
	{
		std::string code_str = Parser::Unparse(en, enm, false, true);
	...
	else //has comments, so need to thoughtfully handle showing first line of comments and appropriate amount of code
	{
		...
		std::string code_str = Parser::Unparse(&en_without_comment, enm, false, true);
		ClampSingleLineStringLength(code_str, max_num_chars);
	...
	while(true)
	{
		...
		else if(command == "p" || command == "pv" || command == "pp")
		{
			...
			else //valid string
			{
				...
				{
					if(command == "p")
						std::cout << Parser::Unparse(node, evaluableNodeManager, true, true) << std::endl;
					else if(command == "pv")
						std::cout << Parser::Unparse(node, evaluableNodeManager, false, true) << std::endl;
					else if(command == "pp")
					{
						std::string var_preview = Parser::Unparse(node, evaluableNodeManager, false, true);
		...
		else if(command == "eval")
		{
			...
			std::cout << Parser::Unparse(result, evaluableNodeManager, true, true) << std::endl;
