...
*/
...

	{
		...
		"output" : "*",
		"new value" : "new",
		"description" : "Performs a deep copy on data (a copy of all data structures referenced by it and its references), then looks at the remaining parameters as pairs.  For each pair, the first is any of: a number, representing an index, with negative numbers representing backward traversal from the end of the list; a string, representing the index; or a list, representing a way to walk into the structure as the aforementioned values as a walk_path. new_value1 to new_valueN represent a value that will be used to replace  whatever is in the location the preceding location parameter specifies. If a particular location does not exist, it will be created assuming the most generic type that will support the index (as a null, list, or assoc); however, it will not change the type of immediate values to an assoc or list. Note that the target operation will evaluate to the new copy of data, which is the base of the newly constructed data; this is useful for creating circular references.",
...

	{
		...
		"output" : "*",
		"description" : "Evaluates to the current node that is being iterated over, or the base code of a set or replace that is being created.  Useful for serializing graph data structures or looking up data during iteration.  If stack_distance is specified, it climbs back up the target stack that many levels, but if the parameter is a boolean, then true indicates the top of the stack and false indicates the bottom.  If walk_path is specified, it will look up the walk_path from the corresponding target stack.  If building an object, specifying top_of_stack to true can be useful for accessing or traversing the top level elements.",
	...
	{
		"parameter" : "query_nearest_generalized_distance list|number entities_returned list axis_labels list axis_values [number p_value] [list|assoc|assoc of assoc weights] [list|assoc distance_types] [list|assoc attributes] [list|assoc deviations] [string weights_selection_feature] [string|number distance_transform] [string entity_weight_label_name] [number random_seed] [string radius_label] [string numerical_precision] [* output_sorted_list]",
		"output" : "query",
		"new value" : "partial",
