...
*/
...

	{
		...
		"output" : "bool",
		"permissions" : "r",
		"description" : "Stores the entity specified by the id to the resource in string. Returns true if successful, false if not. If file_type is specified and not null, it will use the file_type specified instead of the extension of the file_path.  File formats supported are amlg, json, yaml, csv, and caml; anything not in this list will be loaded as a binary string.  Note that loading from a non-'.amlg' or non-'.caml' extension will only ever provide lists, assocs, numbers, and strings.  If params is specified, it is an assoc that contains key-value pairs describing the format.  The key \"sort_keys\" can be used to specify a boolean value, if true, then it will sort the keys, otherwise the default behavior is to emit the keys based on memory layout.  The following keys can be used when storing amlg or caml files.  The key \"include_rand_seeds\" indicates whether random seeds will be stored, which defaults to true.  The key \"parallel_create\" indicates whether creating entities will be performed in parallel when the caml is loaded, which defaults to false.  The parameter escape_filename defaults to false, but if true, it will agressively escape filenames using only alphanumeric characters and the underscore, using underscore as an escape character.  If escape_contained_filenames is true, which is its default, it will escape contained entity filenames.  The parameter flatten, if true which is default, will flatten the entity before storing, which means the code should be executed on load to recreate the entity structure.",
